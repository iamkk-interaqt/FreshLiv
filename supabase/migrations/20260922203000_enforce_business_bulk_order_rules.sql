-- Enforce FreshLiv business bulk rules at order creation.
-- Business orders may only contain products explicitly enabled for bulk ordering.
-- Recurring bulk orders require an active FreshLiv Plus subscription.
create or replace function public.create_customer_order(
  p_dairywala_id uuid,
  p_address_id uuid,
  p_delivery_slot_id uuid,
  p_items jsonb,
  p_customer_type text default 'HOME',
...
begin
  ...
  if p_customer_type='BUSINESS' and p_order_type='ONE_TIME_BULK' then
    v_fee := coalesce(public.get_monetization_amount('bulk_one_time_fee'),99);
  elsif p_customer_type='BUSINESS' and p_order_type='RECURRING_BULK' then
    if not public.is_gwalawala_plus(v_uid) then
      raise exception 'FreshLiv Plus is required for recurring bulk orders' using errcode='42501';
    end if;
    v_fee := 0;
  end if;
  ...
  values(v_uid,p_dairywala_id,p_address_id,p_delivery_slot_id,'PAYMENT_PENDING',0,0,0,p_customer_type,p_order_type,p_purchase_category,p_purchase_requirement,v_fee)
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    select id,price,dairywala_id,status::text as status_txt,name,bulk_order_enabled into v_prod_row from public.products where id=v_product_id;
    ...
    v_price := coalesce(v_prod_row.price,0);
  ...
  if to_regclass('public.order_status_history') is not null then
    insert into public.order_status_history(order_id,from_status,to_status,actor_user_id,note) values(v_order_id,null,'PAYMENT_PENDING',v_uid,'Placed via customer app');
  end if;
  return v_order_id;
end
$function$;
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(); }} />}>
      ...
      {orders.map((order) => (
        <View key={order.id} style={styles.card}>
          ...
          <Text style={styles.timeline}>Current status: {order.status.replaceAll('_', ' ').toLowerCase()}</Text>
        </View>
      ))}
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  title: { marginTop: 6, marginBottom: 24, fontSize: 32, fontWeight: '800' },
  ...
  timeline: { marginTop: 14, fontSize: 13, color: '#555' },
});

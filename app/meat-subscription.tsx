...
export default function MeatSubscriptionScreen(){
 ...
 const plus=()=>{setPurchaseContext({customerType:'BUSINESS',orderType:'RECURRING_BULK',category:p.category,requirement:'Recurring Bulk Supply'});if(plusActive){router.push({pathname:'/meat-discovery',params:{...p,productCategory:p.category,usage:'SHOP',variant:p.variant,bulkOnly:'true',orderType:'RECURRING_BULK',subscriptionPlan:'FreshLiv Plus'}});}else{router.push({pathname:'/gwalawala-plus',params:{...p,category:p.category,requirement:'Recurring Bulk Supply',vertical:'MEAT'}});}};

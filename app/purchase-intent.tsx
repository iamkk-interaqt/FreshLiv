...
export default function PurchaseIntentScreen(){
 ...
 return <ScrollView contentContainerStyle={styles.container}>
   ...
   {customerType==='HOME'&&requirement&&category==='MILK'?<View style={styles.section}>
   ...
   {customerType==='HOME'?<View style={styles.section}>
    <Text style={styles.sectionTitle}>What do you use it for?</Text>
   ...
   </View>:customerType==='BUSINESS'?<View style={styles.section}>
    <Text style={styles.sectionTitle}>Business bulk ordering</Text>
    <Text style={styles.businessBody}>Choose one-time bulk ordering or FreshLiv Plus recurring supply. Product-specific choices come next.</Text>
    <Pressable style={[styles.orderCard,orderType==='ONE_TIME_BULK'&&styles.selected]} onPress={()=>setOrderType('ONE_TIME_BULK')}><Text style={styles.orderTitle}>One-Time Bulk Order</Text><Text style={styles.orderBody}>₹99 one-time convenience fee</Text></Pressable>
    <Pressable style={[styles.orderCard,orderType==='RECURRING_BULK'&&styles.selected]} onPress={()=>setOrderType('RECURRING_BULK')}><View style={styles.plusRow}><Text style={styles.orderTitle}>FreshLiv Plus</Text><Text style={styles.plusPill}>⭐ PLUS</Text></View><Text style={styles.orderBody}>₹499/month · ₹0 service fee on eligible recurring bulk orders</Text></Pressable>
   </View>:null}
   {customerType&&(customerType==='BUSINESS'?orderType:requirement)?<Pressable style={styles.button} onPress={continueFlow}><Text style={styles.buttonText}>{customerType==='BUSINESS'&&orderType==='RECURRING_BULK'?'Continue to FreshLiv Plus':'Continue'}</Text></Pressable>:null}
 </ScrollView>;
}
